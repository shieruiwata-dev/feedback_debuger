...
#### 方法 A: DSL をインポートする（推奨・1 分）

`docs/dify/feedback-classifier.yml` に完成した DSL を置いてある。

1. Dify のアプリ一覧 → **アプリを作成する** → **DSL ファイルをインポート**
2. `docs/dify/feedback-classifier.yml` をアップロード → **作成**
3. **LLM ノードのモデルを自社で使えるものに差し替える**
   （DSL には `openai / gpt-4o-mini` が入っている。Dify 側で未設定のプロバイダなら
   ノードが赤くなるので、選び直す。temperature は 0 のままにする）
4. 右上 **公開する** → **APIアクセス** から API キー（`app-...`）を発行

DSL の中身:

| ノード | 役割 |
|---|---|
| 開始 | `feedback_text`（段落・必須）/ `app_name`（短文・任意） |
| 分類 | ノイズ判定・論点分割・優先度/カテゴリ/要約を 1 回の LLM 呼び出しで行う |
| 終了 | 出力変数 `result`（LLM の text をそのまま） |

**出力変数名は `result` から変えないこと。** Edge Function が `data.outputs.result` を読む。

#### 方法 B: 手で作る

DSL のインポートが Dify のバージョン差で通らない場合はこちら。
